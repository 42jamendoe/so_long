...
&game->tile.wall.endian);
	...
	!game->tile.player.addr || !game->tile.floor.addr || \
	!game->tile.exit.addr || !game->tile.wall.addr)
		exit(EXIT_FAILURE);
}
...
PATH_WALL, &game->tile.wall.width, &game->tile.wall.height);
	if (!game->tile.collectible.ptr || !game->tile.collectible.ptr || \
!game->tile.player.ptr || !game->tile.floor.ptr || !game->tile.exit.ptr)
		exit(EXIT_FAILURE);
}
...
void	ft_mlx_init(t_game *game)
{
	...
	game->size.y * game->tile_size, GAME_TITLE);
	if (!game->window)
	{
		free(game->mlx);
		free(game->window);
